...
const ScanRequestSchema = z.object({
  ...
  query: z.string().default('in:inbox is:unread'),
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Rate limiting check
    const now = Date.now();
    const userRequest = requestCounts.get(session.user.id) || { count: 0, lastReset: now };
    ...
    if (userRequest.count >= RATE_LIMIT.MAX_REQUESTS) {
      return NextResponse.json(
        ...
        { 
          status: 429,
          headers: {
            'Retry-After': String(Math.ceil((userRequest.lastReset + RATE_LIMIT.WINDOW_MS - now) / 1000)),
            'X-RateLimit-Limit': String(RATE_LIMIT.MAX_REQUESTS),
            'X-RateLimit-Remaining': '0',
            'X-RateLimit-Reset': String(Math.ceil((userRequest.lastReset + RATE_LIMIT.WINDOW_MS) / 1000)),
          },
        }
      );
    }
    ...
    if (accounts.length === 0) {
      return NextResponse.json(
        { 
          success: false, 
          error: 'No active Gmail accounts found. Please connect a Gmail account first.' 
        },
        { status: 400 }
      );
    }

    console.log(`[Email Scan] Starting scan for user ${session.user.id} with ${accounts.length} accounts`);
    ...
    const batchPromises = accounts.map(account => 
      (async () => {
        ...
        try {
          ...
          accountLogger(`Completed scan in ${duration}ms: ${JSON.stringify(result)}`);
          ...
          try {
            ...
            console.error(`[${account.email}] Failed to update account status:`, dbError);
          }
    ...
    const summary = results.reduce(
      (acc, result) => {
        ...
        acc.totalProcessed += result.processed || 0;
        acc.totalSkipped += result.skipped || 0;
        acc.totalErrors += result.errors || 0;
        return acc;
      },
      { 
        completed: 0, 
    ...
    console.log(`[Email Scan] Completed scan for user ${session.user.id} in ${totalDuration}ms`, {
